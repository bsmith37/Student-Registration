...
public class TimeRange {
    ...
    public int getDuration() {
		...
		int hour = (endTime.getHour() - startTime.getHour()) * 60;
		int min = endTime.getMinute() - startTime.getMinute();
    ...
    public boolean contains(Time time) {
		...
		if(this.startTime.before(time) && this.endTime.after(time)){
			return true;
		} else if(this.startTime.equals(time) || this.endTime.equals(time)){
			return true;
		} else{
			return false;
		}
    ...
    public boolean overlaps(TimeRange otherRange) {
        ...
        if(startTime.after(otherRange.getStartTime()) && startTime.before(otherRange.getEndTime())){
			return true;
		}
		else if(endTime.after(otherRange.getStartTime()) && endTime.before(otherRange.getEndTime())){
			return true;
		}
		else if(startTime.equals(otherRange.getEndTime()) || endTime.equals(otherRange.getStartTime())){
			return true;
		}
		else if(startTime.equals(otherRange.getStartTime()) || endTime.equals(otherRange.getEndTime())){
		    return true;
		}
